...
export default function Strategies() {
  ...
  const rollDice = () => {
    ...
    setShowingReveal(false); // Ensure reveal states are reset on new roll
  ...
  const checkPlayerMoves = () => {
    ...
    for (let i = 0; i < correctMoves.length; i++) {
      ...
      // Splitting by space handles multiple moves like "24/23 13/11"
      const correctMoveSet = new Set(correctMove.name.split(' '));
  ...
  const handleNewOpening = () => {
    // Auto-roll dice for new opening instead of going to waiting state
    let die1, die2;
    do {
      die1 = Math.floor(Math.random() * 6) + 1;
      die2 = Math.floor(Math.random() * 6) + 1;
  ...
  const tryAgain = () => {
    ...
    setGameState('playing');
    setFeedback(null);
    setMovesRemaining([...dice]); // Re-use the original dice for the 'Try Again' attempt
    setShowingReveal(false);
    setRevealComplete(false);
  };
  ...
  const handleReveal = () => {
    ...
    // Get the first correct move sequence
    const correctMove = correctMoves[0];
    const moveSequence = correctMove.name.split(' ');
    
    // Execute moves sequentially to show the reveal
      ...
      <div className="min-h-screen p-4" style={{ backgroundColor: '#e5e4cd' }}>
        <div className="max-w-6xl mx-auto space-y-4">
          ...
          <div className="text-center">
            <h1 className="text-2xl mb-2 font-bold" style={{ color: '#5a3217' }}>Strategy Practice</h1>
          </div>
          ...
          <div className="text-center">
            <Card className="elegant-shadow border-0 inline-block" style={{ backgroundColor: '#9fd3ba' }}>
              <CardHeader><CardTitle className="text-sm" style={{ color: '#5a3217' }}>STATUS</CardTitle></CardHeader>
              <CardContent className="flex flex-col items-center gap-2 min-h-[5rem] justify-center">
                {gameState === 'waiting' ? (
                  <Badge variant="outline" className="px-4 py-2 bg-yellow-100 text-yellow-800">
                    Click Roll to Start
                  </Badge>
                ) : gameState === 'playing' ? (
                  <Badge variant="outline" className="px-4 py-2 bg-blue-100 text-blue-800">
                    <BookOpen className="w-4 h-4 mr-2" />Make Strategic Move
                  </Badge>
                ) : feedback ? (
                  <div className="flex flex-col items-center gap-2">
                    <Badge variant="outline" className={`px-4 py-2 ${feedback.isCorrect ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                      {feedback.isCorrect ? <><CheckCircle className="w-4 h-4 mr-2" />Correct!</> : <><XCircle className="w-4 h-4 mr-2" />Incorrect</>}
                    </Badge>
                    {/* The Try Again button is now handled by BackgammonBoard via onTryAgain prop */}
                  </div>
                ) : null}
              </CardContent>
            </Card>
          ...
          <Card className="elegant-shadow border-0" style={{ backgroundColor: '#9fd3ba' }}>
            ...
            <CardContent>
              <ul className="text-sm space-y-1" style={{ color: '#5a3217' }}>
                ...
                <li>• Complete both moves to see if you made the optimal strategic play!</li>
              </ul>
            </CardContent>
          </Card>

          <Card className="elegant-shadow border-0" style={{ backgroundColor: '#9fd3ba' }}>
            <CardHeader><CardTitle style={{ color: '#5a3217' }}>STRATEGIC PRINCIPLES</CardTitle></CardHeader>
            <CardContent>
